...
const Contant = () => {
  ...
  const fetchdata = async () => {
    ...
    await apiAdminConfig
      .get(`room/getAllRoomsOfClinicWithClincDataAndOccupancy`)
      .then((res) => {
        if (res.status === 200) {
  ...
  var currentPost = posts
    .filter(
      (el) =>
        ...
        el.percent.toString().includes(query.toLowerCase()) ||
        el.totalRoom.toString().includes(query.toLowerCase()) ||
        el.occupiedRoomId.toString().includes(query.toLowerCase()) ||
        el.emptyRoom.toString().includes(query.toLowerCase())
    )
          ...
          <Row className="mt-4">
            <Col md={6} style={{ textAlign: "left" }}>
              <h3 style={{ height: "40px" }}>
                Clinic Utilization ({query ? currentPost.length : posts.length})
              </h3>
            </Col>
            <Col md={4}>
...
class ReportUtils extends Component {
  constructor(props) {
    super(props);
    this.state = {
