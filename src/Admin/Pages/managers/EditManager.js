...
const Contant = () => {
      ...
      await apiAdminConfig
        ...
        .then((response) => {
          if (response && response?.status === 200) {
            ...
            setTimeout(() => {
              navigate(`/master/receptionist`, { replace: true });
            }, 1000);
          }
        })
        .catch((error) => {
  ...
  const fetchmanager = async (value) => {
    ...
    await apiAdminConfig(`clinicManager/${value}`)
      .then((response) => {
        if (response.status === 200) {
          console.log("response", response);
          let apiData = response.data.data;
          formik.setFieldValue("receptionistName", apiData.clinic_manager_name);
          formik.setFieldValue("email", apiData.email);
          formik.setFieldValue(
            "clinicId",
            apiData.clinicId ? apiData.clinicId._id : ""
          );
        }
      })
      .catch((error) => console.log("error", error));
        ...
        <>
          ...
          <Row className="mt-4">
            <Col md={6} style={{ textAlign: "left" }}>
              <h3 style={{ height: "40px" }}>Edit Clinic Manager</h3>
            </Col>
            <Col md={4}></Col>
