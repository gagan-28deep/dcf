...
// import { LocationSearchBox } from "../../../Components/locationsearch/LocationSearchBox";
const Contant = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setErrors }) => {
      console.log(values, selected.length);
      values.status = true;
      setLoader(true);
      await apiAdminConfig
        ...
        .then((response) => {
          if (response.status === 200 || response.status === 201) {
            setLoader(false);
            snackbar({
              message: "Clinc has been created successfully.",
              severity: "success",
            });
            formik.resetForm();
            setSelected([]);
  ...
  // Get lat long
  const getCord = (value) => {
    console.log('getlocationLat', value);
        ...
        <>
          <Row className="mt-4">
            <Col md={6} style={{ textAlign: "left" }}>
              <h3 style={{ height: "40px" }}>Add Clinic</h3>
            </Col>
            <Col md={4}></Col>
